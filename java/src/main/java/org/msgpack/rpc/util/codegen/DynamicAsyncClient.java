...
public class DynamicAsyncClient {
    private static Logger LOG = LoggerFactory
            .getLogger(DynamicAsyncClient.class);

    private static DynamicAsyncClientCodeGen gen;
    ...
    public static Object create(Client client, Class<?> handlerType) {
        LOG.info("create an instance of " + DynamicAsyncClient.class.getName()
                + ": handler type: " + handlerType.getName());
        if (gen == null) {
            gen = new DynamicAsyncClientCodeGen();
        }
